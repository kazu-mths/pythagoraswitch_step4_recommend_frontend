...
export default function QrcodeReaderComponent() {
    ...
    async function fetchUser(token: string) {
        const res = await fetch(`/shopping?token=${user_token}`, { cache: "no-cache" });
        if (!res.ok) {
            throw new Error('Failed to fetch user');
        }
        return res.json();
    ...
    useEffect(() => {
        if (router.isReady) {
            // URLからtokenを取得して状態にセット
            const user_token = router.query.token;

            // user_tokenが文字列の場合にのみsetTokenを更新
            if (typeof user_token === 'string') {
                setToken(user_token);
            } else {
                // user_tokenが未定義または文字列の配列の場合はnullまたは適切なデフォルト値をセット
                setToken(null);
            }
        }
    ...
    async function fetchProduct(scannedResult: any) {
        const encodedQrcode = encodeURIComponent(scannedResult);
        const res = await fetch(`/qrcode?qrcode=${encodedQrcode}`, { cache: "no-cache" });
        if (!res.ok) {
            throw new Error('Failed to fetch product');
        }
        return res.json();
    ...
    const fetchAndDealDetail = async (buyTime: Date) => {
        ...
        const response = await fetch('/deal_detail', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
